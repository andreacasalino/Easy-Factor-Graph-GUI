...
std::unique_ptr<gui::json::streamJSON> GraphShell::getNetworkJSON() {
  ...
  if(nullptr != this->graph) {
    ...
    for(auto it = hiddenVars.begin(); it!=hiddenVars.end(); ++it) {
      ...
      if(itMap == this->lastMap.end()) {
        ...
        addNode((*it)->name() + "=" + std::to_string(itMap->second) + " (MAP)", (*it)->name(), IMAGE_FOLDER + "Variable.svg");
      }
    }
    auto obVars = this->graph->getEvidences();
    for(auto it = obVars.begin(); it!=obVars.end(); ++it) {
      addNode(it->first->name() + "=" + std::to_string(it->second), it->first->name(), IMAGE_FOLDER + "Variable_Observed.svg");
    }
  }
  ...
  auto addEdge = [&edges, &nodes, &counter, &addNode](const EFG::distribution::Distribution& pot, const std::string& image) {
  ...
  if(nullptr != this->graph) {
    auto factors = this->graph->getFactors();
    for (auto it = factors.begin(); it != factors.end(); ++it) {
        addEdge(**it, IMAGE_FOLDER + "Potential_Shape.svg");
    }
    auto factorsExp = this->graph->getFactorsExp();
...
std::string GraphShell::CreateIsolatedVar(const gui::RequestOptions& opt){
  ...
  EFG::categoric::VariablePtr var = std::make_shared<EFG::categoric::Variable>(std::atoi(itS->second[0].c_str()), itV->second[0]);
...
std::string GraphShell::GetVariableInfo(const gui::RequestOptions& opt) {
  ...
  inspectRes->addElement("i", gui::json::Number<bool>( this->isolatedVars.find(info) != this->isolatedVars.end()) );
...
EFG::categoric::VariablePtr GraphShell::findVariable(const std::string& name) const {
    auto itIso = this->isolatedVars.find(std::make_unique<EFG::categoric::Variable>(2 , name));
...
bool GraphShell::AddFactor(const std::string& name, const std::string& fileName, const float& weight) {
  auto info = this->findVariable(name);
  if(nullptr == info) return false;
  try {
    std::shared_ptr<EFG::distribution::factor::cnst::Factor> pot = std::make_shared<EFG::distribution::factor::cnst::Factor>(std::set<EFG::categoric::VariablePtr>{info}, fileName);
    if(nullptr == this->graph) this->graph = std::make_unique<EFG::model::Graph>();
    if(0.f == weight) {
      this->graph->insert(pot);
    }
    else {
      this->graph->insert(std::make_shared<EFG::distribution::factor::cnst::FactorExponential>(*pot, weight));
    }
  }
  catch(...) {
    return false;
...
bool GraphShell::AddFactor(const std::string& nameA, const std::string& nameB, const std::string& fileName, const float& weight) {
  auto infoA = this->findVariable(nameA);
  if(nullptr == infoA) return false;
  auto infoB = this->findVariable(nameB);
  if(nullptr == infoB) return false;
  try {
    std::shared_ptr<EFG::distribution::factor::cnst::Factor> pot = std::make_shared<EFG::distribution::factor::cnst::Factor>(std::set<EFG::categoric::VariablePtr>{infoA, infoB}, fileName);
    if(nullptr == this->graph) this->graph = std::make_unique<EFG::model::Graph>();
    if(0.f == weight) {
      this->graph->insert(pot);
    }
...
bool GraphShell::AddFactor(const std::string& nameA, const std::string& nameB, const bool& corr_anti, const float& weight) {
    auto infoA = this->findVariable(nameA);
    if (nullptr == infoA) return false;
    auto infoB = this->findVariable(nameB);
    if (nullptr == infoB) return false;
  try {
      std::shared_ptr<EFG::distribution::factor::cnst::Factor> pot = std::make_shared<EFG::distribution::factor::cnst::Factor>(std::set<EFG::categoric::VariablePtr>{infoA, infoB}, corr_anti);
    if(nullptr == this->graph) this->graph = std::make_unique<EFG::model::Graph>();
    if(0.f == weight) {
      this->graph->insert(pot);
    }
